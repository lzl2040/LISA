...
class ConditionEmbedder(nn.Module):
    ...
    """
    def __init__(self, in_size, hidden_size, dropout_prob=0.1, conditions_shape=(1, 1, 256)):
        super().__init__()
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        patch_size=2,
        in_channels=4,
        condition_in_channels=256,
        img_feats_channels=4096,
        hidden_size=1152,
        depth=28,
        num_heads=16,
        mlp_ratio=4.0,
        ...
        self.z_embedder = ConditionEmbedder(in_size=condition_in_channels, hidden_size=hidden_size, dropout_prob=class_dropout_prob)
        self.img_condition_embedder = nn.Linear(img_feats_channels, hidden_size)
        # self.y_embedder = 
    ...
    def initialize_weights(self):
        ...
        # nn.init.normal_(self.z_embedder.embedding_table.weight, std=0.02)
    ...
    def forward(self, x, t, z, img_feats):
        """
        Forward pass of DiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        z: (N, C) tensor of class labels
        img_feats: (N L D)
        """
        x = self.x_embedder(x) + self.pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
        t = self.t_embedder(t)                   # (N, D)
        z = self.z_embedder(z, self.training)    # (N, D)
        condition_img = self.img_condition_embedder(img_feats) # B L D
        c = t + z                               # (N, D)
        c = torch.cat([c, condition_img], dim = 1)
        # print(c.shape)
        # print(f"condition shape:{c.shape} x shape:{x.shape}") # c: [1, 1, 384],  x: [1, 57600, 384]
        for block in self.blocks:
            x = block(x, c)                      # (N, T, D)
